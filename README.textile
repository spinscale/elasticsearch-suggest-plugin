 ...
 * Run: <code>mvn clean package</code> - this does not run any unit tests, as they take some time. If you want to run them, better run <code>mvn clean package -DskipTests=true</code>
...
* Cleanup of suggester structures on stopping the instance, deleting the shard, moving the shard from one node to another
* Ensure that transport actions are not loaded twice
* Allow clearing of fst suggesters at least per index or per field
* Add tests for refreshing only certain fields or indexes
* Update documentation for index/field refresh
...
* Tests: Start nodes in parallel via futures so the tests for clustered environments run faster
