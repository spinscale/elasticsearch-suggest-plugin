...
The plugin only works with the most simple setup of one shard on one node.
Feel free to comment, improve and help - I am thankful for any insights.
 ...
 * Add debugging
 * Test the SuggestService instead of the JSON response
 * Parse the JSON arguments like in the ES actions (currently the size parameter as string can spit exceptions)
 * Extend the client interface and create a SuggestRequest java class - is this possible at all with a plugin
 ...
 * Support multiple shards on one node
 * Support multiple shards over multiple nodes
