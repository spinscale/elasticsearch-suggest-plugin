...
<pre><code>bin/plugin -url https://github.com/downloads/spinscale/elasticsearch-suggest-plugin/elasticsearch-suggest-0.0.9-0.19.9.zip -install suggest</code></pre>
 ...
 * Install the plugin: <code>/path/to/elasticsearch/bin/plugin -install elasticsearch-suggest -url file:///$PWD/build/distributions/elasticsearch-suggest-$version.zip</code>
...
NodesSuggestRefreshResponse response = node.client().execute(SuggestRefreshAction.INSTANCE, refreshRequest).actionGet();
</code></pre>
...
* Querying by using a TransportClient should work (otherwise pushing it to a maven repo does not make too much sense)
* Find the current resource leak (open deleted files after lots of merging)
* Use FSTCompletionLookup instead of deprecated FSTLookup (since Lucene 3.6) - seems to break tests currently
