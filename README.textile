 ...
 * Check Shays response and incorporate comments
   * Call searcher.release() in finally statement in order to release resources
   * Index should not get refreshed on each change (implement periodic update, also implement rest call to refresh)
 ...
 * Find out whether the indexreader hashmap leaks memory (possibly)
 * Take care of case when the indexreader has been closed
 * Find out why there is a one second delay in response while there are lots of updates - and how to remove it
 * Make nicer tests
 * Really test the SuggestService instead of the JSON response, integration test is nice, but not what I want
 * What about supporting certain types only, this is completely ignored for now
 * How to extend node.client() to return a node client with suggest support? Is this possible without hard patching?
